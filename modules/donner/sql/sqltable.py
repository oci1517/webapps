...
class SQLTable(Table):
    ...
    def run(self):
        ...
        table_node['classes'] += self.options.get('class', []) + 'table-striped table-bordered table-hover table-condensed ocidbtable'.split(' ')

        self.add_name(table_node)
        if title:
            table_node.insert(0, title)
        #else:
        #    table_node.insert(0, nodes.paragraph(text=u"Résultat de la requête", attributes={'classes':['table-sql-result']}))
